...
import six
import yaml
from vnxCliApi.common import Dict, Cache
from vnxCliApi.enums import Enum, VNXSPEnum
import logging
...
class Converter(object):
    ...
    @classmethod
    def str_to_int_arr(cls, str_input):
        ...
        ints = map(to_int_silent_error, str_input.split(','))
    ...
    @staticmethod
    def to_sp_enum(value):
        return VNXSPEnum.from_str(value)
...
def _get_resource_module():
    res_module = 'vnxCliApi.resources'
    if res_module not in sys.modules:
        __import__(res_module)
    module = sys.modules[res_module]
    return module
    ...
    def get_converter(converter_str):
        ...
        if converter_str is not None:
            ...
            if len(converter_desc_list) == 1:
                ...
                # default to class `Converter`
                try:
                    cls = getattr(sys.modules[__name__], 'Converter')
                    ret = getattr(cls, converter)
