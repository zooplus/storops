...
from vnxCliApi.exception import VNXFsExistedError
from vnxCliApi.lib.common import check_int
from vnxCliApi.vnx.enums import raise_if_err, VNXError
import vnxCliApi.vnx.resource.mover
import vnxCliApi.vnx.resource.nas_pool
...
class VNXFileSystem(VNXResource):
    ...
    def create(cli, name, pool, size_kb=None, mover=1, is_vdm=False):
        ...
        pool = vnxCliApi.vnx.resource.nas_pool.VNXNasPool.get_id(pool)
        mover = vnxCliApi.vnx.resource.mover.VNXMover.get_id(mover)
    ...
    def extend(self, new_size, pool=None):
        if pool is None and self.pools:
            pool = self.pools[0]
        pool_id = vnxCliApi.vnx.resource.nas_pool.VNXNasPool.get_id(pool)
