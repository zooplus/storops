...
import vnxCliApi.vnx.resource.fs
...
class VNXFsSnap(VNXResource):
    ...
    @property
    def fs(self):
        clz = vnxCliApi.vnx.resource.fs.VNXFileSystem
        return clz(fs_id=self.fs_id, cli=self._cli)

    @classmethod
    def create(cls, cli, name, fs, pool, size=None):
        fs_clz = vnxCliApi.vnx.resource.fs.VNXFileSystem
        pool_clz = vnxCliApi.vnx.resource.nas_pool.VNXNasPool
