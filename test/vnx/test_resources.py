...
from hamcrest import assert_that, equal_to, contains_string, is_not, \
    has_item, none, raises, only_contains, instance_of, has_items, is_in
from test.vnx.test_parsers import STORAGE_GROUP_HBA

from test.vnx.cli_mock import patch_cli, read_test_file
from vnxCliApi.exception import VNXModifyLunError, VNXConsistencyGroupError, \
    ...
    VNXCreateRaidGroupError, VNXRemoveRaidGroupError, VNXCreatePoolError, \
    VNXRemovePoolError
from vnxCliApi.lib.common import cache
from vnxCliApi.vnx.cli import CliClient
from vnxCliApi.vnx.enums import \
    VNXPortType, VNXProvisionEnum, VNXTieringEnum, VNXSPEnum, \
    VNXMigrationRate, VNXCompressionRate, VNXMirrorViewRecoveryPolicy, \
    VNXMirrorViewSyncRate, VNXLunType, VNXRaidType
from vnxCliApi.vnx.parsers import get_parser_config
from vnxCliApi.vnx.resources import \
    ...
    VNXMigrationSession, VNXNdu, VNXMirrorView, VNXMirrorViewImage, VNXDisk, \
    VNXRaidGroup
...
@cache()
def test_vnx():
    return VNXSystem('10.244.211.30')
...
class VNXPoolTest(TestCase):
    ...
    def test_create_pool(self):
        def f():
            VNXPool.create(test_cli(), 'p0', ['1_0_0', '1_0_1'])
...
class VNXMirrorViewTest(TestCase):
    @patch_cli()
    def test_get_all(self):
        mv_list = VNXMirrorView.get(test_cli())
    ...
    def test_mirror_view_not_installed(self):
        ...
        mv = VNXMirrorView.get(test_cli(), 'mv_sync_2')
...
class VNXDiskTest(TestCase):
    ...
    @patch_cli()
    def test_get_all(self):
        disks = VNXDisk.get(test_cli())
        assert_that(len(disks), equal_to(180))
        for disk in disks:
            if disk.index == '4_0_e8':
                self.validate_disk_4_0_e8(disk)
                break

    @staticmethod
    def validate_disk_4_0_e8(disk):
    ...
    @patch_cli()
    def test_get_disk(self):
        disk = VNXDisk.get(test_cli(), '4_0_e8')
        self.validate_disk_4_0_e8(disk)

    @patch_cli()
    def test_remove_disk(self):
        disk = VNXDisk('0_0_1', test_cli())
        ret = disk.remove()
...
class VNXRaidGroupTest(TestCase):
    ...
    def test_get_rg(self):
        rg = VNXRaidGroup.get(test_cli(), 0)
        self.verify_raid0(rg)

    @patch_cli()
    def test_get_rg_list(self):
        rgs = VNXRaidGroup.get(test_cli())
    ...
    def test_create_rg(self):
        def f():
            VNXRaidGroup.create(test_cli(), 11, ['1_0_0', '1_0_1'])
    ...
    def test_create_rg_invalid_raid_type(self):
        def f():
            VNXRaidGroup.create(test_cli(), 11, ['1_0_0', '1_0_1'], 'r12')
        assert_that(f, raises(ValueError, 'valid value'))

    @patch_cli()
    def test_remove_rg(self):
        def f():
            VNXRaidGroup(11, test_cli()).remove()
