...
mock_ssh_connector = mock.patch.object(connector, "SSHConnector",
                                       mock.Mock())
...
class FakeData(object):
    ...
    filesystem_size = 10 * Ki
    filesystem_new_size = 20 * Ki
...
class MountPointTestData(StorageObjectTestData):
    ...
    @response
    def resp_get_succeed(self, mover_id, is_vdm=True, path=None):
        if not path:
            path = self.path
...
class MoverTestData(StorageObjectTestData):
    ...
    def resp_get_succeed(self, name=None):
        ...
        return (
            ...
            '<MoverInterface name="%(long_interface_name)s" device="cge-2-0" '
            ...
            '<MoverInterface name="%(interface_name1)s" device="%(device)s" '
            'ipVersion="IPv4" netMask="255.255.255.0" '
            'broadcastAddr="192.168.1.255" macAddr="0:60:16:53:cc:87" '
            'mtu="1500" up="true" vlanid="%(vlan_id)s" mover="%(id)s" '
            'ipAddress="%(ip_addr1)s"/>'
            ...
            'ipAddress="%(ip_addr2)s"/>'
            '<MoverInterface name="fake_interface_name" device="mge-1-0" '
            'ipVersion="IPv4" netMask="255.255.255.0" '
            'broadcastAddr="128.221.252.255" macAddr="0:60:16:53:cc:87" '
            'mtu="1500" up="true" vlanid="0" mover="fake_mover_id" '
            'ipAddress="%(ip_addr2)s"/>'
            ...
            % {'id': self.mover_id,
               'name': name,
               'long_interface_name': self.long_interface_name[:32],
               'interface_name1': self.interface_name1,
               'interface_name2': self.interface_name2,
               'ip_addr1': self.ip_address1,
               'ip_addr2': self.ip_address2,
               'vlan_id': six.text_type(self.vlan_id),
...
class CIFSShareTestData(StorageObjectTestData):
    ...
    def cmd_change_access(self, access_level='rw',
                          action='grant'):
